# Keeping for reference - Script requires a PAT token from DevOps.  
# Also requires Visual Studio to be installed on the machine running the script.
...
[string]$ExcelFile= '"C:\Git\learn-tools\freshness\test.xlsx"'
...
[string]$areapath="Content\Production\Data and AI\Machine Learning"
[string]$iterationpath="Content\Future"
[string]$assignee=''
[string]$parentItem="319589" #the ADO parent feature to link the new items to. Empty string if there is none.

## Locate the Visual Studio install on your PC
[string]$dllpath = "C:\Program Files\Microsoft Visual Studio\2022\Community\Common7\IDE\CommonExtensions\Microsoft\TeamFoundation\Team Explorer"
...
[string]$mode="freshness"  # or "engagement"
...
{
[array]$tags = @('content-health','freshness','Scripted')
...
[string]$defaultTitle= "Freshness - over 90:  "
}
...
try{
    ...
    foreach($row in $allRowsArray)
    {
        ...
        $description += "<br/><a href={0} target=_new>{0}</a><br/>" -f $row["LiveUrl"] 
        $description += "<table style='border: 1px solid black; border-collapse: collapse;'>"

        if ($mode -ceq "freshness")
        {
            $description += "<tr><td align='right' style='border: 1px solid black; border-collapse: collapse;'><strong>Freshness</strong></td><td align='left' style='border: 1px solid black; border-collapse: collapse;'> {0}</td></tr>" -f $row["Freshness Bucket"]
            ...
            $description += "<tr><td align='right' style='border: 1px solid black; border-collapse: collapse;'><strong>PageViews</strong></td><td align='left' style='border: 1px solid black; border-collapse: collapse;'> {0}</td></tr>" -f $row["PageViews"]
        }
        ...
        $item.Title = $defaultTitle + $row["Title"]
