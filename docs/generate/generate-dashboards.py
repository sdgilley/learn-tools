...
def get_snippet_files(file_path):
    ...
    with open(file_path, "r") as file:
        ...
        ]  # Get the file name, not the codeowners
    return lines
...
def get_tutorial_files(file_path):
    ...
    with open(file_path, "r") as file:
        ...
        for line in file:  # Read the rest of the lines
            line = line.split(",")[0]
            line = line.replace(
                "https://raw.githubusercontent.com/Azure/azureml-examples/", ""
            )
...
def write_html(notebooks):
    ...
    gh_link = (
        "https://github.com/Azure/azureml-examples/blob/main"  # Where to find the files
    )
    rows_by_extension = {}
    ...
    for notebook in notebooks:
        ...
        # get the name of the notebook file
        ...
        rows_by_extension.setdefault(
            extension, []
        )  # Initialize an empty list if this extension hasn't been seen before

        file = notebook.split("/")[-1].replace(
            ".", "&#46;"
        )  # Last part of the path is the file name
        file_name = os.path.splitext(file)[0]
        file = file.replace("%20", " ").replace(
            "&#46;", "."
        )  # Put back the spaces and dots in the file name for better readability
    ...
    with open(file_path, "w") as file:
        ...
        file.write(
            "<div class='small-table'><table><tr><th>File type</th><th class='number'>#</th></tr>\n"
        )
        for extension, rows in sorted(rows_by_extension.items()):
            if extension:
                ext = extension[1:].strip()  # Get rid of the leading dot
                file.write(
                    f"<tr><td><a href='{ext}.html'>{extension}</a></td><td class='number'>{len(rows)}</td></tr>\n"
                )  # Add the 'number' class to the cell
        file.write(
            f'<tr><td><a href="all.html">Total</a></td><td class="number">{sum(len(rows) for rows in rows_by_extension.values())-1}</td></tr>\n'
        )
    ...
    for extension, rows in sorted(rows_by_extension.items()):
        if extension:
            ...
            with open(file_path, "w") as file:
                ...
                file.write(
                    f'<p class="update">{len(rows)} files. Last update: {today}</p>\n'
                )
